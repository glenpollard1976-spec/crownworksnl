...
try {
    ...
    Write-Host "✅ Anthropic SDK installed" -ForegroundColor Green
    Write-Host ""
} catch {
    Write-Host "⚠️ Anthropic SDK installation issue (may already be installed)" -ForegroundColor Yellow
    Write-Host ""
}
...
try {
    ...
    Write-Host "✅ Google Gemini SDK installed" -ForegroundColor Green
    Write-Host ""
} catch {
    Write-Host "⚠️ Google Gemini SDK installation issue (may already be installed)" -ForegroundColor Yellow
    Write-Host ""
}
...
if (Test-Path $envPath) {
    ...
    Write-Host "⚠️ .env.local not found, will create it" -ForegroundColor Yellow
    Write-Host ""
}
...
Write-Host "  $($hasOpenAI ? '✅' : '❌') OpenAI API Key"
Write-Host "  $($hasAnthropic ? '✅' : '❌') Anthropic API Key"
Write-Host "  $($hasGoogle ? '✅' : '❌') Google API Key"
Write-Host ""
...
if (-not $hasAnthropic -or -not $hasGoogle) {
    ...
    Write-Host "✅ .env.local updated with placeholder keys" -ForegroundColor Green
    Write-Host "⚠️ Remember to replace placeholder keys with actual API keys!" -ForegroundColor Yellow
    Write-Host ""
}
...
try {
    ...
    Write-Host "⚠️ Could not test providers (this is normal if keys are not set yet)" -ForegroundColor Yellow
    Write-Host ""
}
